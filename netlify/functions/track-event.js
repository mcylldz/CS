...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (customer) {
      ...
      if (customer.city) userData.ct = [sha256Geo(customer.city)];
      if (customer.state) userData.st = [sha256Geo(customer.state)];
      if (customer.zip) userData.zp = [sha256Geo(customer.zip)];
